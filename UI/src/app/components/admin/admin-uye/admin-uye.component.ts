...
import { MesajDialogComponent } from '../../dialogs/mesaj-dialog/mesaj-dialog.component';
...
export class AdminUyeComponent implements OnInit {
  ...
  dialogRefMesaj: MatDialogRef<MesajDialogComponent>;
  ...
  MesajGonder(kayit: Uye) {
    this.dialogRefMesaj = this.matDialog.open(MesajDialogComponent, {

    })

    this.dialogRefMesaj.afterClosed().subscribe(d => {
      if (d) {
        this.apiServis.MesajGonder(kayit.userId, d.mesaj).subscribe((s: Sonuc) => {
          this.alert.AlertUygula(s)
