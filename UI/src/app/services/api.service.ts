...
export class ApiService {
  ...
  CevapListeByMakaleId(soruId: number) {
    return this.http.get(this.apiUrl + 'cevap-liste-by-soru-id/' + soruId);
  }

  CevapEkle(cevap: Cevap) {
    return this.http.post(this.apiUrl + 'cevap-ekle/', cevap);
  }
  ...
  MesajGonder(uyeId: string, mesaj: string) {
   return  this.http.post(this.apiUrl + "mesaj-gonder", {
      kimdenId: localStorage.getItem("uid"),
      kimeId: uyeId,
      icerik: mesaj,
      tarih: new Date(Date.now()).toLocaleString("tr")
    })
  }

  MesajListele() {
    return this.http.get(this.apiUrl + "mesaj-by-kime-id/" + localStorage.getItem("uid"))
  }
}
